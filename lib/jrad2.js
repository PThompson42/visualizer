...
class ApplicationDirector {
  ...
  selectFile (fileCallback, accept = '') {
    ...
    inputElement.addEventListener('change', this.fileDialogChanged.bind(this))
...
class AO {
  ...
  listenForClicks (callback) {
    this.element.addEventListener('click', this.clickCallback.bind(this), false)
  ...
  listenForInput (callback) {
    this.element.addEventListener('input', this.inputCallback.bind(this), false)
  ...
  handleDrop (e) {
    let draggedItemDetails = JSON.parse(e.dataTransfer.getData('text/plain'))
    let details = {
      note: 'drop',
      e: e,
      offset: { x: e.offsetX, y: e.offsetY },
...
class UIContainer extends AO {
  ...
  animRender () {
    ...
    if (this.bLimitFramerate && this.anim.timeGap < 1000 / this.anim.fps) return
...
class SpinButton extends AO {
  constructor (
    ...
    super({ id: id, parentID: parentID })
    //console.log(this.id);
    this.label = new Label('lbl_' + this.id, startval, this.id).alignCenter()
    this.loval = loval
    this.hival = hival
...
function processColor () {
  ...
  if (arguments.length == 0) {
    ...
    if (typeof t == 'string') {
      ...
      let str = 'rgb(' + String(t) + ',' + String(t) + ',' + String(t) + ')'
      return str
    }
    ...
    if (alpha > 255) {
      console.error('Error in process color - alpha must be between 0 and 1')
      return '#000000'
    }
    if (typeof t == 'string') {
      if (t.charAt(0) != '#') {
...
class Vector {
  ...
  minus (v) {
    if (this.rank !== v.rank) alert('cannot subtract vectors of unequal rank')
    return new Vector(
      ...this.vector.map((item, index) => {
        return item - v.vector[index]
      })
...
class Pos2d extends Vector {
  ...
  updatePosFromDisplay () {
    this.x =
      Pos2d.viewCenter.x + (this.display.x - Pos2d.screenCenter.x) / Pos2d.zoom
    this.y =
      -1 *
