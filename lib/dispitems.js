class Fix {
  ...
  constructor (f, dispX, dispY, settings, displayContainer, displayList) {
    ...
    this.w = settings.baseFixSize / 2
    this.h = settings.baseFixSize / 2
  ...
  draw (settings, bNeedsUpdate, itemSelected) {
    ...
    if (this.fixType == 'bmp') {
      ...
      if (this.fixType == 'square') {
        ...
        let ctx = cv.ctx
        ctx.beginPath()
        ctx.moveTo(this.pos.display.x - dSize, this.pos.display.y + dSize)
        ctx.lineTo(this.pos.display.x, this.pos.display.y - dSize)
        ctx.lineTo(this.pos.display.x + dSize, this.pos.display.y + dSize)
        ctx.closePath()
        ctx.stroke()
        ctx.fill()
...
class Runway {
  constructor (
    ...
    //set initial colors
    this.weight = settings.rwyStrokeWeight
    this.strokeStyle = settings.rwyStrokeColor
    this.fillStyle = settings.rwyFillColor
  ...
  dragHandle (index, dX, dY) {
    ...
    if (index == 1 || index == 3) {
      ...
      this.pos.x = (this.aControlPoints[1].x + this.aControlPoints[3].x) / 2
      this.pos.y = (this.aControlPoints[1].y + this.aControlPoints[3].y) / 2
      dispatchMessage(MSGREDRAW, true)
      return
    }
  ...
  draw (settings, bNeedsUpdate, itemSelected) {
    ...
    ctx.beginPath()
    ctx.moveTo(this.aIconVertices[0].display.x, this.aIconVertices[0].display.y)
    ctx.lineTo(this.aIconVertices[1].display.x, this.aIconVertices[1].display.y)
    ctx.lineTo(this.aIconVertices[2].display.x, this.aIconVertices[2].display.y)
    ctx.lineTo(this.aIconVertices[3].display.x, this.aIconVertices[3].display.y)
    ctx.closePath()
    ctx.stroke()
    ctx.fill()
...
class Approach {
  constructor (
    ...
    this.weight = settings.approachStrokeWeight
    this.strokeStyle = settings.approachStrokeColor
